...
function use-case-add-remove-static-zone-and-record() {
	...
	./static-zone-rr-create.sh "$DDNS_NAME_SERVER" "$DDNS_ZONE" "test-host-${RANDOM_STRING_6}" "$ip_address" "600" "/etc/bind/${DDNS_ZONE}_NSUPDATE.key"

	echo "# ACTION delete record"
	./static-zone-rr-delete.sh "$DDNS_NAME_SERVER" "$DDNS_ZONE" "test-host-${RANDOM_STRING_6}" "/etc/bind/${DDNS_ZONE}_NSUPDATE.key"
